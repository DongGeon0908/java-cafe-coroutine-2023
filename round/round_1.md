...
**Outer Scope의 변수를 접근할 수 있는 func을 Closure라고 함.**

람다식 내에서 자유변수가 사용되는 것을 클로저라고 한다. <u>람다 표현식이라고 국한 짓는 것은 잘못됨</u>

자유변수는 람다식 외부에 존재하며, 별도로 파라미터로 전달 받지 않은 값을 의미한다. (외부 메서드에 존재하지만, 외부 메서드의 종료에 영향을 받지 않고 자유롭게 사용 가능)
...
java의 클로저와 유사하다. 중요한 점은 아래의 개념이라고 생각한다.



- Free Variable : 다식 내에서 정의되지 않고, 또 파라미터로도 전달되지 않은 외부 Local variable를 참조하는 변수

- Variable Capture : 람다식에서 사용한 데이터를 자유변수에 저장하는 것
- Lambda Capturing : 람다식에서 자유 변수를 참조하는 행위
